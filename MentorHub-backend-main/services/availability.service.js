...
const getMentorAvailabilityForNext14Days = async (
  ...
  try {
    ...
    if (!mentorAvailability) {
      console.log(`No availability found for mentor: ${userId}`);
      return []; // Return empty array if no availability set
    }
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (!dailyAvailability || !Array.isArray(dailyAvailability) || dailyAvailability.length === 0) {
        continue;
      }
      ...
      if (slotsForDay.length > 0) {
        next14DaysAvailability.push({
          id: `day-${i}`,
          date: currentDate,
          slots: slotsForDay.map((slot, slotIndex) => ({
            ...slot,
            id: `slot-${i}-${slotIndex}`,
          })),
        });
